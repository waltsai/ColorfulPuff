...
import waltsai.colorfulpuff.server.entity.ai.task.*;

import java.util.Optional;

public class PuffBrain {
    ...
    public static Brain<PuffEntity> create(PuffEntity entity, Brain<PuffEntity> brain) {
        ...
        brain.setSchedule(ModProvider.PUFF_ADULT);
    ...
    private static boolean canRide(PuffEntity puff, Entity ridden) {
        if (ridden instanceof MobEntity mobEntity) {
            return !mobEntity.isAlive() || PuffBrain.hasBeenHurt(puff);
        }
        return false;
    }
    ...
    private static Task<PuffEntity> makeFollowEntitiesTask() {
        return new RandomTask<>(ImmutableList.of(Pair.of(new TimeLimitedTask<>(new FollowNonLivingEntityTask(4.0F), UniformIntProvider.create(40, 80)), 4), Pair.of(new TimeLimitedTask<>(new FollowMobTask(PuffBrain::isNonPlayerMobEntity, 4.0F), UniformIntProvider.create(40, 80)), 4), Pair.of(new TimeLimitedTask<>(new FollowMobTask(EntityType.PLAYER, 4.0F), UniformIntProvider.create(40, 160)), 8)));
    }

    private static Task<PuffEntity> makeBusyFollowTask() {
        return new RandomTask<>(ImmutableList.of(Pair.of(new FollowMobTask(ModProvider.PUFF_ENTITY, 8.0F), 2), Pair.of(new FollowMobTask(EntityType.PLAYER, 8.0F), 2), Pair.of(new WaitTask(30, 60), 8)));
    }

    private static TimeLimitedTask<PuffEntity> makeRememberRideableAnimalTask() {
        return new TimeLimitedTask<>(new MemoryTransferTask<>(PuffEntity::isBaby, ModProvider.NEAREST_VISIBLE_RIDABLE_ANIMALS, MemoryModuleType.RIDE_TARGET, RIDE_TARGET_MEMORY_DURATION), MEMORY_TRANSFER_TASK_DURATION);
    }

    private static Task<PuffEntity> makeRandomStrollTask(boolean isVivid) {
        if(!isVivid) {
            return new RandomTask<>(ImmutableList.of(Pair.of(new StrollTask(1.0F), 2), Pair.of(FindEntityTask.create(ModProvider.PUFF_ENTITY, 8, MemoryModuleType.INTERACTION_TARGET, 1.0F, 2), 2), Pair.of(new GoTowardsLookTarget(0.6F, 3), 2), Pair.of(new WaitTask(40, 80), 4)));
        }
        return new RandomTask<>(ImmutableList.of(Pair.of(new StrollTask(1.06F), 2), Pair.of(FindEntityTask.create(ModProvider.PUFF_ENTITY, 8, MemoryModuleType.INTERACTION_TARGET, 1.06F, 2), 2), Pair.of(new GoTowardsLookTarget(0.6F, 3), 2), Pair.of(new WaitTask(40, 80), 2)));
    }
    ...
    public static Optional<BlockPos> findNearestDangerousBlock(World world, BlockPos pos, int range) {
        return BlockPos.findClosest(pos, range, 4, (blockPos) -> world.getBlockState(blockPos).isIn(ModProvider.PUFF_REPELLENTS));
    }
}
