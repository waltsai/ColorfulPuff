package waltsai.colorfulpuff.server.entity.ai.brain.sensor;
...
import waltsai.colorfulpuff.server.entity.PuffBrain;
...
public class PuffSpecificSensor extends Sensor<PuffEntity> {
    ...
    public Set<MemoryModuleType<?>> getOutputMemoryModules() {
        return ImmutableSet.of(MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.NEAREST_REPELLENT, MemoryModuleType.TEMPTING_PLAYER, ModProvider.VISIBLE_INTERESTED_ENTITIES, ModProvider.NEAREST_VISIBLE_RIDABLE_ANIMALS);
    }

    protected void sense(ServerWorld serverWorld, PuffEntity puff) {
        Brain<?> brain = puff.getBrain();
        brain.remember(MemoryModuleType.NEAREST_REPELLENT, PuffBrain.findNearestDangerousBlock(serverWorld, puff.getBlockPos(), 8));
        ...
        brain.remember(ModProvider.VISIBLE_INTERESTED_ENTITIES, new ArrayList<>(list2));
        ...
        for (LivingEntity livingEntity : livingTargetCache.iterate(livingEntity -> true)) {
            if (livingEntity instanceof AnimalEntity animal && ridable(animal)) {
                brain.remember(ModProvider.NEAREST_VISIBLE_RIDABLE_ANIMALS, Optional.of(animal));
                break;
            }
        }
    }
